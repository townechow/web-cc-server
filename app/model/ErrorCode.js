...
module.exports = app => {
  ...
  const ReportSchema = new Schema({
    ...
    ip: { type: String },
    recordScreenId: { type: String },
    message: { type: String },
    line: { type: Number }, // 
    column: { type: Number }, // 
    fileName: { type: String },//报错文件 


  });
  ReportSchema.index({ create_time: 1 });
  app.model.ErrorCode = function (appId) {
    return conn.model(`error_code_${appId}`, ReportSchema);
  };
  // return conn.model('error_code', ReportSchema);
};
