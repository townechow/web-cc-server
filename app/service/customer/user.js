...
class UserService extends Service {
  ...
  async register(userName, passWord) {
    ...
    const user = this.ctx.model.Customer.User();
  ...
  async getUserInfoForUserName(userName) {
    return await this.ctx.model.Customer.User.findOne({ user_name: userName }).exec() || {};
  }
  ...
  async getUserList(pageNo, pageSize, userName) {
    ...
    const count = Promise.resolve(this.ctx.model.Customer.User.count(query).exec());
    const datas = Promise.resolve(
      this.ctx.model.Customer.User.find(query).skip((pageNo - 1) * pageSize)
        .limit(pageSize)
        .exec()
    );
  ...
  async setIsUse(id, isUse, usertoken) {
    // 冻结用户信息
    isUse = +isUse;
    const result = await this.ctx.model.Customer.User.update(
      { _id: id },
      { is_use: isUse },
      { multi: true }
    ).exec();
  ...
  async delete(id, usertoken) {
    // 删除
    const result = await this.ctx.model.Customer.User.findOneAndRemove({ _id: id }).exec();
    // 清空登录态
  ...
  async finUserForToken(usertoken) {
    ...
    return await this.ctx.model.Customer.User.findOne({ token: user_info.token }).exec();
  }
  ...
  async getUserInfoForGithubId(id) {
    return await this.ctx.model.Customer.User.findOne({ token: id }).exec() || {};
  }
